...
while(n.region <= length(regions)) {                                                                                 # Loop Over Regions
  if(regions[n.region] != "North America" & regions[n.region] != "Territory") {                                      # Different setup for North America - Canada and the United States
    dta <- full.dta[full.dta$Region == regions[n.region],]                                                           # Setup Data for the loop by getting data from full.dta
    file.Label <- paste(regions[n.region],"-Results-",Sys.Date())                                                    # Create label for written files
...
# Set First and last date for estimation purposes                                                                    #
    ...
    first.date <- dates[length(dates)-120]                                                                            # Beginning of period of analysis (first point for estimation)
#                                                                                                                    #
# Create test values to see that the country can be added                                                            #
    test.Stats <- lapply(cntlst,function(i) list(mean(dta[(dta$Country == i & dta$Date >= first.date &               # Test.Stats[1] - Mean of Population
                                                           dta$Date <= last.date),"Population.100K"]),               #
                                                 mean(dta[(dta$Country == i & dta$Date >= first.date &               # Test.Stats[2] - Mean of Cases.Daily
                                                           dta$Date <= last.date),"Cases.Daily"]),                   #
                                                 mean(dta[(dta$Country == i & dta$Date >= first.date &               # Test.Stats[3] - Mean of Tests Daily
                                                           dta$Date <= last.date),"Tests.Daily"]),                   #
                                                 mean(dta[(dta$Country == i & dta$Date >= first.date &               # Test.Stats[4] - Mean of Cumulative Cases
                                                           dta$Date <= last.date),"Total.Cases"]),                   #
                                                 mean(dta[(dta$Country == i & dta$Date >= first.date &               # Test.Stats[5] - Mean of Cumulative Deaths
                                                           dta$Date <= last.date),"Total.Deaths"]),                  # 
                                                 mean(dta[(dta$Country == i & dta$Date >= first.date &               # Test.Stats[6] - Mean of Cumulative Tests
                                                           dta$Date <= last.date),"Total.Tests"])))                  #                                                                                                                     #
...
# Compute basic data transformations                                                                                 #
    ...
    for(i in 1:nrow(dta)) {                                                                                          # Loop through complete dataset
      ...
      dta$Trend[i] <- as.numeric(dta$Date[i] - dates[1])                                                             # 
      if(dta$Date[i] >= (last.date - 14)) dta$RecentWeek[i] <- 1                                                     # Set change two weeks before last date
    }                                                                                                                #
...
# Econometrics of COVID model                                                                                        #
                                                                                                                     #
    p.dta <- pdata.frame(dta[dta$Date >= (first.date + 59),],index=c("Country","Date"))                              # Create a panel dataset from the original data
                                                                                                                     #
# --------------------------------------- DPD estimation ------------------------------------------------------------#
                                                                                                                     #
    if(regions[n.region] != "South Asia" & regions[n.region] != "Sub-Saharan Africa") {                              # Different specification for n.region = 7 (South Asia) and n.region = 8 (Sub-Saharan Africa)
                                                                                                                     #
...
# ----------------------------------- Wald Test for Lag Structure ---------------------------------------------------#
    ...
    } else if(regions[n.region] == "South Asia") {                                                                   #
                                                                                                                     #
      ...
      sink()                                                                                                         #
                                                                                                                     #
    } else if(regions[n.region] == "Sub-Saharan Africa") {                                                           #
      ...
      for(j in 1:7) {                                                                                                #
        ...
        out.tble[n.rws[j],15] <- (out.dta[n.date[j],16,i] + out.dta[n.date[j]-1,16,i] + out.dta[n.date[j]-2,16,i] +  # 7 Day Persistence
                                  ...
                                  out.dta[n.date[j]-6,16,i])/7.0                                                     #
                                                                                                                     ...
                                                                                                                     #
    write.csv(file=paste(file.Label," Weekly.csv"),out.tble)                                                                #
                                                                                                                     #
# End of country                                                                                                     #
######################################################################################################################
                                                                                                                     #
      ...
      for(j in 1:7) {                                                                                                #
        ...
        out.tble[n.rws[j],15] <- (out.dta[n.date[j],16,i] + out.dta[n.date[j]-1,16,i] + out.dta[n.date[j]-2,16,i] +  # 7 Day Persistence
                                  ...
                                  out.dta[n.date[j]-6,16,i])/7.0                                                     #
                                                                                                                     ...
                                                                                                                     #
#                                                                                                                    #
    write.csv(file=paste("United States -Results-",Sys.Date()," Weekly.csv"),out.tble)                                      #
#                                                                                                                    #
...
write.csv(file = paste("Canada -Results-",Sys.Date()," Weekly.csv"),out.tble)                                               #
#                                                                                                                    #
# End of Canada                                                                                                      #
######################################################################################################################    
  }
