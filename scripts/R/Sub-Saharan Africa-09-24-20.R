...
# Read Primary Data                                                                                                  #
                                                                                                                     #
dta <- read.xlsx(xlsxFile="C:/Users/janin/Downloads/ssa.xlsx",na.string="NA",sheet=1)                                #
                                                                                                                     #
...
for(i in 1:nrow(dta)) {                                                                                              # Loop through complete dataset
  ...
  if(dta$Date[i] >= 44083) dta$WeekM0[i] <- 1
  else if(dta$Data[i] >= 44076) dta$WeekM1[i] <- 1
}
...
p.dta <- pdata.frame(dta,index=c("Country","Date"))                                                                  # Create a panel dataset from the original data
                                                                                                                     #
# -- DPD estimation                                                                                                  #
gmm0 <- pgmm(Pos.Rate ~ lag(Pos.Rate,c(1,7)) + WeekM0 + WeekM1 + Cum.Tests + Weekend | Trend + I(Trend^2) +          # Estimate DPD model
                        lag(Pos.Rate,8:15) | lag(Pos.Rate,2:5),data=p.dta,effect="individual",model="onestep",       #
...
for(i in 1:length(cntlst.2)) {
  ...
  for(j in 8:l.end) out.dta[j,15,i] <- gmm0$coefficients[1]*out.dta[j-1,2,i] + gmm0$coefficients[3]                    # 1 day persistance effect
  for(j in 8:l.end) out.dta[j,16,i] <- gmm0$coefficients[2]*out.dta[j-7,2,i] + gmm0$coefficients[4]                    # 7 day persistence effect
}
