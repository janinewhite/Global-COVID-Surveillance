...
# Read Primary Data                                                                                                  #
                                                                                                                     #
full.dta <- read.csv(file="daily_raw_input.csv")                                                     # Read data from pgmm_input_recalc_totals.csv file
...
# Create test values to see that the country can be added                                                            #
    test.Stats <- lapply(cntlst,function(i) list(mean(dta[dta$Country == i,"Population.100K"]),                           # S[1] - mean of Population
                                                 mean(dta[dta$Country == i,"Cases.Daily"]),                       # s[2] - mean of Positive Daily
                                                 mean(dta[dta$Country == i,"Tests.Daily"]),                          # s[3] - mean of Tests Daily
                                                 mean(dta[dta$Country == i,"Total.Cases"]),                       # s[4] - mean of Cumulative Positives
                                                 ...
                                                 mean(dta[dta$Country == i,"Total.Tests"])))                         # s[6] - mean of Cumulative Tests
                                                                                                                     #
# Loop to drop suspect countries
...
# Set First and last date for estimation purposes                                                                    #
    dates <- sort(unique(dta$Excel.Date))                                                                                  # List of dates in the dataset
...
# Fill in missing cumulative dta                                                                                     #
    ...
    for(i in 1:length(cntlst)) {                                                                                     # Fill in cumulative positive cases
      if(is.na(test.Stats[[i]][[4]])) {                                                                              #
        ...
        dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[1]),"Total.Cases"] <-                              #
          dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[1]),"Cases.Daily"]                               #
        for(j in 2:length(dates.Temp)) {                                                                             #
          dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[j]),"Total.Cases"] <-                            #
            dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[j-1]),"Total.Cases"]+                          #
            dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[j]),"Cases.Daily"]                             #
        }                                                                                                            #
      }                                                                                                              #
    }                                                                                                                #
    for(i in 1:length(cntlst)) {                                                                                     # Fill in cumulative deaths
      if(is.na(test.Stats[[i]][[5]])) {                                                                              #
        dates.Temp <- sort(unique(dta[dta$Country == cntlst[i],"Date"]))                                             #
        dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[1]),"Total.Deaths"] <-                                #
          dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[1]),"Deaths.Daily"]                                 #
        for(j in 2:length(dates.Temp)) {                                                                             #
          dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[j]),"Total.Deaths"] <-                              #
            dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[j-1]),"Total.Deaths"]+                            #
            dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[j]),"Deaths.Daily"]                               #
        }                                                                                                            #
      }                                                                                                              #
    }                                                                                                                #
    for(i in 1:length(cntlst)) {                                                                                     # Fill in cumulative tests
      if(is.na(test.Stats[[i]][[6]])) {                                                                              #
        dates.Temp <- sort(unique(dta[dta$Country == cntlst[i],"Date"]))                                             #
        dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[1]),"Total.Tests"] <-                                 #
          dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[1]),"Tests.Daily"]                                  #
        for(j in 2:length(dates.Temp)) {                                                                             #
          dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[j]),"Total.Tests"] <-                               #
            dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[j-1]),"Total.Tests"]+                             #
            dta[(dta$Country == cntlst[i] & dta$Excel.Date == dates.Temp[j]),"Tests.Daily"]                                #
        }                                                                                                            #
      }                                                                                                              #
    }                                                                                                                #
                                                                                                                     #
...
# Compute basic data transformations                                                                                 #
    ...
    dta$R.Date     <- convertToDate(dta$Excel.Date,origin = "1900-01-01")                                                  # R.Data - date converted into R format
    ...
    for(i in 1:nrow(dta)) {                                                                                          # Loop through complete dataset
      ...
      dta$Trend[i] <- dta$Excel.Date[i] - dates[1]                                                                         # Compute the trend based on Excel data variable
      if(dta$Excel.Date[i] >= (last.date - 15)) dta$RecentWeek[i] <- 1                                                     # Set change two weeks before last date
    }                                                                                                                #
...
# Econometrics of COVID model                                                                                        #
                                                                                                                     #
    p.dta <- pdata.frame(dta[dta$Excel.Date >= first.date,],index=c("Country","Date"))                                     # Create a panel dataset from the original data
                                                                                                                     #
# --------------------------------------- DPD estimation ------------------------------------------------------------#
                                                                                                                     #
...
#  out.dta[j,16,i] - 7 Day Persistance Effect                                                                        #
    ...
    for(i in 1:(length(cntlst.2)-1)) {                                                                               ###########
      ...
      for(j in 1:l.end) out.dta[j,2,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Cases.Daily"]     # New Cases
      for(j in 1:l.end) out.dta[j,3,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Total.Cases"]     # Cumulative Cases
      ...
      for(j in 1:l.end) out.dta[j,5,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Pos.Rate"]           # Rate of new infections per 100,000
      for(j in 1:l.end) out.dta[j,6,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Deaths.Daily"]       # New COVID deaths
      for(j in 1:l.end) out.dta[j,7,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Total.Deaths"]       # Cumulative COVID deaths
      ...
      for(j in 1:l.end) out.dta[j,9,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Death.Rate"]         # COVID death rate per 100,000
      ...
      for(j in 1:l.end) out.dta[j,17,i] <- dta[(dta$Country == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"RecentWeek"]         # Dummy variable for last two weeks
...
#                                                                                                                              #
    reg.pop <- 0                                                                                                               #
    for(i in 1:length(cntlst)) reg.pop <- reg.pop + dta[(dta$Country == cntlst[i] & dta$Excel.Date == first.date),"Population.100k"] #
    ...
    for(j in 1:l.end) out.dta[j,17,reg.totals] <- dta[(dta$Country == cntlst.2[1] & dta$Excel.Date == out.dta[j,1,i]),"RecentWeek"]  # Recent Week dummy for shift in parameters
...
# Econometrics of COVID model                                                                                        #
                                                                                                                     #
    p.dta <- pdata.frame(dta[dta$Excel.Date >= first.date,],index=c("State.Province","Date"))                              # Create a panel dataset from the original data
                                                                                                                     #
#                                                                                                                    #
# --------------------------------------- DPD estimation ------------------------------------------------------------#
#                                                                                                                    #
...
######################################################################################################################
      ...
      for(i in 1:(length(cntlst.2)-1)) {                                                                             #
        ...
        for(j in 1:l.end) {                                                                                          # New Cases
          out.dta[j,2,i]  <- dta[(dta$State.Province == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Cases.Daily"]} #
        for(j in 1:l.end) {                                                                                          # Cumulative Cases
          out.dta[j,3,i]  <- dta[(dta$State.Province == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Total.Cases"]} #
        ...
        for(j in 1:l.end) {                                                                                          # Rate of new infections per 100,000
          out.dta[j,5,i]  <- dta[(dta$State.Province == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Pos.Rate"]}       #
        for(j in 1:l.end) {                                                                                          # New COVID deaths
          out.dta[j,6,i]  <- dta[(dta$State.Province == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Deaths.Daily"]}   #
        for(j in 1:l.end) {                                                                                          # Cumulative COVID deaths
          out.dta[j,7,i]  <- dta[(dta$State.Province == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Total.Deaths"]}   #  
        ...
        for(j in 1:l.end) {                                                                                          # COVID death rate per 100,000
          out.dta[j,9,i]  <- dta[(dta$State.Province == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"Death.Rate"] }    #   
        for(j in 1:l.end) out.dta[j,10,i] <- out.dta[j,5,i]                                                          # Speed - Rate per 100,000
        for(j in 2:l.end) out.dta[j,11,i] <- out.dta[j,5,i] - out.dta[j-1,5,i]                                       # Change in speed
        ...
        for(j in 1:l.end) {                                                                                          # Dummy variable for last two weeks
          out.dta[j,17,i] <- dta[(dta$State.Province == cntlst.2[i] & dta$Excel.Date == out.dta[j,1,i]),"RecentWeek"] }    # Recent Week dummy
...
#                                                                                                                    #
      ...
      for(i in 1:length(states)) {                                                                                   #
          reg.pop <- reg.pop + dta[(dta$State.Province == states[i] & dta$Excel.Date == first.date),"Population.100k"] }   #
      reg.totals <- length(cntlst.2)                                                                                 # Region Total Column
      for(j in 1:l.end) out.dta[j,1,reg.totals] <- out.dates[j]                                                      # Region Total Date
      for(j in 1:l.end) out.dta[j,2,reg.totals] <- sum(out.dta[j,2,])                                                # Region Total New Cases
      for(j in 1:l.end) out.dta[j,3,reg.totals] <- sum(out.dta[j,3,])                                                # Region Total Cumulative Cases
      ...
      for(j in 1:l.end) {                                                                                            # Recent Week dummy for shift in parameters
        out.dta[j,17,reg.totals] <- dta[(dta$State.Province == cntlst.2[1] & dta$Excel.Date == out.dta[j,1,i]),"RecentWeek"] }    #
