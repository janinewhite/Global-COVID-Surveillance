...
# Read Primary Data                                                                                                  #
                                                                                                                     #
dta <- read.xlsx(xlsxFile="East Asia and the Pacific-10-20-2020.xlsx",na.string="NA",sheet=1)                        #
                                                                                                                     #
cntlst <- c("Australia","Brunei","China","French Polynesia","Guam","Indonesia",
            "Japan","Malaysia","Mongolia","Myanmar","New Zealand",
            "Papua New Guinea","Philippines","Singapore","South Korea","Taiwan","Thailand",               
            "Vietnam")
# cntlst <- unique(dta$Country)

pop <- read.xlsx(xlsxFile="East Asia and the Pacific-10-20-2020.xlsx",na.string="NA",sheet=2)                        #
                                                                                                                     #
                                                                                                                     #
dta$Pop <- 0                                                                                                         #
for(i in 1:length(cntlst)) {                                                                                         #
  ...
  for(j in 1:length(dates)) {                                                                                        #
    ...
    if(is.na(dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Daily.Tests"]) == TRUE) {
      dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Daily.Tests"] <- 0 }
    if(is.na(dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Cumulative.Tests"]) == TRUE) {
      dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Cumulative.Tests"] <- 0 }
    if(is.na(dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Daily.Tests"]) == TRUE) {
      dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Daily.Tests"] <- 0 }
  }                                                                                                                  #
}                                                                                                                    #
...
dta$Test.Rate  <- 100000*dta$Daily.Tests/dta$Pop                                                                     # Test.Rate - new tests per 100,000
...
dta$Death.Rate <- 100000*dta$Death.Increase/dta$Pop                                                                  # Death Rate
dta$Cum.Deaths <- 100000*dta$Cumulative.Deaths/dta$Pop                                                               # Cumulative Death
...
p.dta <- pdata.frame(dta[(dta$Date >= strt.date & dta$Date <= end.date),],index=c("Country","Date"))                                          # Create a panel dataset from the original data
                                                                                                                     #
# -- DPD estimation                                                                                                  #
gmm0 <- pgmm(Pos.Rate ~ lag(Pos.Rate,c(1,7)) + Test.Rate + Weekend + Cum.Tests | Trend + I(Trend^2) + lag(Pos.Rate,8:15) |                  #
...
for(i in 1:(length(cntlst.2)-1)) {
  ...
  for(j in 1:l.end) out.dta[j,7,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Date == out.dta[j,1,i]),"Cumulative.Deaths"] # Cumulative COVID deaths
...
write.csv(file="Pacific Output.csv",out.tble)
