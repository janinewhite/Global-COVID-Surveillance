...
# Read Primary Data                                                                                                  #
                                                                                                                     #
dta <- read.xlsx(xlsxFile="Middle East and North Africa (MENA)-10-21-2020.xlsx",na.string="NA",sheet=1)              #
                                                                                                                     #
cntlst <- c("Algeria","Bahrain","Djibouti","Egypt","Iran","Iraq","Israel","Jordan","Lebanon","Libya",
            "Morocco","Oman","Qatar","Saudi Arabia","Tunisia","United Arab Emirates") 
# cntlst <- unique(dta$Country)

pop <- read.xlsx(xlsxFile="Middle East and North Africa (MENA)-10-21-2020.xlsx",na.string="NA",sheet=2)              #
                                                                                                                     #
                                                                                                                     #
dta$Pop <- 0                                                                                                         #
for(i in 1:length(cntlst)) {                                                                                         #
  dates <- sort(dta[dta$Country==cntlst[i],"Date"])
  for(j in 1:length(dates)) {                                                                                        #
    dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Pop"] <- pop[pop$Country == cntlst[i],"Population"]       #
    if(is.na(dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Positive.Daily"]) == TRUE) {
      dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Positive.Daily"] <- 0 }
    if(is.na(dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Positive.Total"]) == TRUE) {
      dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Positive.Total"] <- 0 }
    if(is.na(dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Tests.Daily"]) == TRUE) {
      dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Tests.Daily"] <- 0 }
    if(is.na(dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Tests.Total"]) == TRUE) {
      dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Tests.Total"] <- 0 }
  }                                                                                                                  #
}                                                                                                                    #
                                                                                                                     #
######################################################################################################################
# Compute basic data transformations                                                                                 #
                                                                                                                     #
dta$Pos.Rate   <- 100000*dta$Positive.Daily/dta$Pop                                                                       # Pos.Rate - new infections per 100,000
dta$Test.Rate  <- 100000*dta$Tests.Daily/dta$Pop                                                                     # Test.Rate - new tests per 100,000
dta$Cum.Tests  <- 100000*dta$Tests.Total/dta$Pop                                                                # Cum.Tests - cumulative tests per 100,000
dta$Cum.Cases  <- 100000*dta$Positive.Total/dta$Pop                                                                             # Cumulative cases
dta$Death.Rate <- 100000*dta$Deaths.Daily/dta$Pop                                                                  # Death Rate
dta$Cum.Deaths <- 100000*dta$Deaths.Total/dta$Pop                                                               # Cumulative Death
...
for(i in 1:nrow(dta)) {                                                                                              # Loop through complete dataset
  ...
  if(dta$Date[i] >= 44097) dta$WeekM0[i] <- 1
  else if(dta$Date[i] >= 44090) dta$WeekM1[i] <- 1
}
...
# Compute sample statistics for a period in time                                                                     #
                                                                                                                     #
strt.date <- 44093                                                                                                   # Start date excel.date(44023) = 7/11/20 (basically two month lag) [44043]
end.date  <- 44122
                                                                                                                     #
...
# Econometrics of COVID model                                                                                        #
                                                                                                                     #
p.dta <- pdata.frame(dta[(dta$Date >= strt.date & dta$Date <= end.date),],index=c("Country","Date"))                 # Create a panel dataset from the original data
                                                                                                                     #
# -- DPD estimation                                                                                                  #
gmm0 <- pgmm(Pos.Rate ~ lag(Pos.Rate,c(1,7)) + Test.Rate + Weekend + Cum.Tests  | Trend + I(Trend^2) +               #
               lag(Pos.Rate,8:15) | lag(Pos.Rate,2:3),data=p.dta,effect="individual",model="onestep",                #
               transformation="ld",fsm="I",na.action=omit,digits=10,robust=TRUE)                                     #
...
for(i in 1:(length(cntlst.2)-1)) {
  ...
  for(j in 1:l.end) out.dta[j,3,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Date == out.dta[j,1,i]),"Positive.Total"] # Cumulative Cases
  ...
  for(j in 1:l.end) out.dta[j,7,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Date == out.dta[j,1,i]),"Deaths.Total"]   # Cumulative COVID deaths
  ...
  for(j in 8:l.end) out.dta[j,15,i] <- gmm0$coefficients[1]*out.dta[j-1,5,i]    # 1 day persistance effect
  for(j in 8:l.end) out.dta[j,16,i] <- gmm0$coefficients[2]*out.dta[j-7,5,i]    # 7 day persistence effect
}
...
for(j in 8:l.end) out.dta[j,15,reg.totals] <- gmm0$coefficients[1]*out.dta[j-1,5,reg.totals]    # 1 day persistance effect
for(j in 8:l.end) out.dta[j,16,reg.totals] <- gmm0$coefficients[2]*out.dta[j-7,5,reg.totals]    # 7 day persistence effect
...
write.csv(file="Middle East Output.csv",out.tble)

s3 <- lapply(cntlst,function(i) list(summary(dta[(dta$Country == i & dta$Date >= strt.date),c("Pos.Rate","Test.Rate","Cum.Tests")]),
                                     table(is.na(dta[(dta$Country == i & dta$Dta >= strt.date),]))))
