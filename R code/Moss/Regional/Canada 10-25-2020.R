...
# Read Primary Data                                                                                                  #
                                                                                                                     #
dta <- read.xlsx(xlsxFile="Canada 10-23-2020.xlsx",na.string="NA",sheet=1)                                           #
                                                                                                                     #
cntlst <- c("Alberta","British Columbia","Manitoba","New Brunswick","Newfoundland and Labrador",                     #
            "Northwest Territories","Nova Scotia","Ontario","Prince Edward Island","Quebec",               #
            "Saskatchewan","Yukon")                                                                                  #
                                                                                                                     #
dta$Country <- dta$Province                                                                                          # Simple solution
pop <- read.xlsx(xlsxFile="Canada 10-23-2020.xlsx",na.string="NA",sheet=2)                                           #
                                                                                                                     #
...
dta$Death.Rate <- 100000*dta$Deaths.Total/dta$Pop                                                                    # Death Rate
dta$Cum.Deaths <- 100000*dta$Deaths.Daily/dta$Pop                                                                    # Cumulative Death
...
for(i in 1:nrow(dta)) {                                                                                              # Loop through complete dataset
  ...
  if(dta$Date[i] >= 44117) dta$WeekM0[i] <- 1
  else if(dta$Date[i] >= 44110) dta$WeekM1[i] <- 1
}
...
# Compute sample statistics for a period in time                                                                     #
                                                                                                                     #
strt.date <- 44073                                                                                                   # Start date excel.date(44023) = 7/11/20 (basically two month lag) [44043]
                                                                                                                     #
...
write.csv(file="Canada Ouput.csv",out.tble)
