...
# Read Primary Data                                                                                                  #
                                                                                                                     #
dta <- read.xlsx(xlsxFile="South ASia-11-9-2020.xlsx",na.string="NA",sheet=1)                                        #
                                                                                                                     #
cntlst <- c("Afghanistan","Bangladesh","Bhutan","India","Maldives","Nepal","Pakistan","Sri Lanka")                   #
                                                                                                                     #
pop <- read.xlsx(xlsxFile="South ASia-11-9-2020.xlsx",na.string="NA",sheet=2)                                        #
                                                                                                                     #
...
for(i in 1:length(cntlst)) {                                                                                         #
  ...
  for(j in 1:length(dates)) {                                                                                        #
    ...
    if(is.na(dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Daily.pos"]) == TRUE) {
      dta[(dta$Country == cntlst[i] & dta$Date == dates[j]),"Daily.pos"] <- 0 }
...
dta$Cum.Cases  <- 100000*dta$cumulative.cases/dta$Pop                                                                # Cumulative cases
...
for(i in 1:nrow(dta)) {                                                                                              # Loop through complete dataset
  ...
  if(dta$Date[i] >= 44128) dta$RecentWeek[i] <- 1
}
...
# Compute sample statistics for a period in time                                                                     #
                                                                                                                     #
strt.date <- 44083                                                                                                   # Start date excel.date(44023) = 7/11/20 (basically two month lag)
                                                                                                                     #
...
p.dta <- pdata.frame(dta,index=c("Country","Date"))                                                                  # Create a panel dataset from the original data
                                                                                                                     #
# -- DPD estimation                                                                                                  #
gmm0 <- pgmm(Pos.Rate ~ lag(Pos.Rate,c(1,7)) + Test.Rate + TestRate.Sq + Cum.Tests + RecentWeek +                    # Estimate DPD model
                        + Weekend | Trend + I(Trend^2) +lag(Pos.Rate,8:15) |                                         #
                        lag(Pos.Rate,2:5),data=p.dta,effect="individual",model="onestep",                            #
...
for(i in 1:(length(cntlst.2)-1)) {
  ...
  for(j in 1:l.end) out.dta[j,3,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Date == out.dta[j,1,i]),"cumulative.cases"] # Cumulative Cases
...
out.tble <- matrix(0,6*length(cntlst.2),15)
n.date <- c(11,18,25,32,39,46)
for(i in 1:length(cntlst.2)) {
  n.rws <- (seq(6*(i-1)+1,6*i,1))
  for(j in 1:6) {
...
write.csv(file="South Asia - Updated.csv",out.tble)
