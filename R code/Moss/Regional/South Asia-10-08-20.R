...
# Read Primary Data                                                                                                  #
                                                                                                                     #
dta <- read.xlsx(xlsxFile="South Asia Data-10-01-2020V2.xlsx",na.string="NA",sheet=1)                                #
                                                                                                                     #
...
# Compute basic data transformations                                                                                 #
                                                                                                                     #
dta$Pos.Rate   <- 100000*dta$Pos.daily/dta$Population                                                                # Pos.Rate - new infections per 100,000
dta$Test.Rate  <- 100000*dta$Daily.Test/dta$Population                                                               # Test.Rate - new tests per 100,000
dta$Cum.Tests  <- 100000*dta$Cumlative.Tests/dta$Population                                                          # Cum.Tests - cumulative tests per 100,000
dta$Cum.Cases  <- 100000*dta$Pos/dta$Population                                                                      # Cumulative cases
dta$Death.Rate <- 100000*dta$Death.Increase/dta$Population                                                           # Death Rate
dta$Cum.Deaths <- 100000*dta$Deaths/dta$Population                                                                   # Cumulative Death
...
for(i in 1:nrow(dta)) {                                                                                              # Loop through complete dataset
  ...
  else if(dta$Data[i] >= 44090) dta$WeekM1[i] <- 1
}
...
p.dta <- pdata.frame(dta[dta$Date >= strt.date,],index=c("Country","Date"))                                          # Create a panel dataset from the original data
                                                                                                                     #
# -- DPD estimation                                                                                                  #
gmm0 <- pgmm(Pos.Rate ~ lag(Pos.Rate,c(1,7)) + WeekM0 + WeekM1 + Test.Rate + Weekend + Cum.Tests | Trend +          # Estimate DPD model
                        I(Trend^2) + lag(Pos.Rate,8:15) | lag(Pos.Rate,2:3),data=p.dta,effect="individual",          #
                        model="onestep",transformation="ld",fsm="I",na.action=omit,digits=10,robust=TRUE)            #
...
for(i in 1:length(cntlst.2)) {
  ...
  for(j in 1:l.end) out.dta[j,2,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Date == out.dta[j,1,i]),"Pos.daily"]      # New Cases
...
write.csv(file="SouthAsia-updated.csv",out.tble)
