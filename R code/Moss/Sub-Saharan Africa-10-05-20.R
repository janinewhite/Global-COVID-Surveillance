...
# Read Primary Data                                                                                                  #
                                                                                                                     #
dta <- read.xlsx(xlsxFile="Sub-Saharan Africa-9-16-2020.xlsx",na.string="NA",sheet=1)                                #
                                                                                                                     #
...
p.dta <- pdata.frame(dta,index=c("Country","Date"))                                                                  # Create a panel dataset from the original data
                                                                                                                     #
# -- DPD estimation                                                                                                  #
gmm0 <- pgmm(Pos.Rate ~ lag(Pos.Rate,c(1,7)) + WeekM0 + Cum.Tests + Weekend | Trend + I(Trend^2) +                   # Estimate DPD model
...
summary(gmm0)                                                                                                        # Print results of DPD estimation
                                                                                                                     #
gmm.cov <- vcov(gmm0)                                                                                                #
gmm.rest <- rbind(cbind(1,0,0,0,0),                                                                                  #
                  cbind(0,1,0,0,0))                                                                                  #
...
write.csv(file="SSA-10-05-20.csv",out.tble)
