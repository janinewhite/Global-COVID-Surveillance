...
# Read Primary Data                                                                                                  #
                                                                                                                     #
dta <- read.xlsx(xlsxFile="Latin America and the Caribbean-10-14-2020.xlsx",na.string="NA",sheet=1)                  #
                                                                                                                     #
cntlst <- c("Antigua & Barbuda","Argentina","Bahamas","Barbados","Belize","Bolivia","Brazil","Chile",                #
            "Colombia","Costa Rica","Cuba","Dominica","Dominican Republic","Ecuador","El Salvador","Grenada",        #
            "Guatemala","Guyana","Haiti","Honduras","Jamaica","Mexico","Panama","Paraguay","Peru",                   #
            "St. Kitts & Nevis","St. Lucia","St. Vincent & Grenadines","Suriname","Trinidad & Tobago","Uruguay",     #
            "Venezuela")                                                                                             #
pop <- read.xlsx(xlsxFile="Latin America and the Caribbean-10-14-2020.xlsx",na.string="NA",sheet=2)                  #
                                                                                                                     #
dates <- unique(dta$Date)                                                                                            #
dta$Pop <- 0                                                                                                         #
...
dta$Test.Rate  <- 100000*dta$new_tests_orig/dta$Pop                                                                  # Test.Rate - new tests per 100,000
dta$Cum.Tests  <- 100000*dta$all_cum_tests/dta$Pop                                                                   # Cum.Tests - cumulative tests per 100,000
dta$Cum.Cases  <- 100000*dta$all_cum_cases/dta$Pop                                                                   # Cumulative cases
dta$Death.Rate <- 100000*dta$DeathIncrese/dta$Pop                                                                    # Death Rate
...
for(i in 1:(length(cntlst.2)-1)) {
  ...
  for(j in 1:l.end) out.dta[j,3,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Date == out.dta[j,1,i]),"all_cum_cases"]  # Cumulative Cases
  ...
  for(j in 1:l.end) out.dta[j,6,i]  <- dta[(dta$Country == cntlst.2[i] & dta$Date == out.dta[j,1,i]),"DeathIncrese"]   # New COVID deaths
...
write.csv(file="LatinAmerica.csv",out.tble)
