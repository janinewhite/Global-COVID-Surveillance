...
# Read Primary Data                                                                                                  #
                                                                                                                     #
dta <- read.xlsx(xlsxFile="Central Asia-10-19-2020.xlsx",na.string="NA",sheet=1)                  #
                                                                                                                     #
cntlst <- c("Armenia","Azerbaijan","Cyprus","Georgia","Kazakhstan","Kosovo","Kyrgyzstan",
            "North Macedonia","Russia","Tajikistan","Turkey","Uzbekistan")                                           #
                                                                                                                     #
pop <- read.xlsx(xlsxFile="Central Asia-10-19-2020.xlsx",na.string="NA",sheet=2)                  #
                                                                                                                     #
...
p.dta <- pdata.frame(dta[dta$Date >= strt.date,],index=c("Country","Date"))                                          # Create a panel dataset from the original data
                                                                                                                     #
# -- DPD estimation                                                                                                  #
gmm0 <- pgmm(Pos.Rate ~ lag(Pos.Rate,c(1,7)) + Test.Rate + Weekend + Cum.Tests + WeekM0 +  # Estimate DPD model
                        WeekM1 | Trend + I(Trend^2) + lag(Pos.Rate,8:15) |                  #
...
write.csv(file="CentralAsia-Results.csv",out.tble)
